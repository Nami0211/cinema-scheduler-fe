#!/usr/bin/env node
/**
 * Bootstrap di milestone, label e issue su GitHub a partire da issues-progetto-cinema-fe.md.
 *
 ...
 * Uso
 * ----
 * In GitHub Actions (consigliato): vedi .github/workflows/bootstrap.yml,
 ...
 * issues-progetto-cinema-fe.md così com'è scritto):
 ...
 *    "### Issue", a un qualsiasi heading "## ", o alla fine del file, ripulito
 *    dai separatori "---".
 *
 * Nota: la chiusura su un heading "## " qualsiasi (non solo "## Milestone") serve
...
function parseIssuesMarkdown(markdown) {
  ...
  for (const line of lines) {
    ...
    // Un "## " che non è una milestone (es. la checklist finale) chiude comunque
    // ciò che è aperto: senza questo, il testo finirebbe in coda all'ultima issue.
    if (anyH2Match) {
      flushIssue();
      flushMilestone();
      continue;
