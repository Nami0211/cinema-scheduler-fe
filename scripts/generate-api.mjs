...
async function checkAndDownloadSpec(url) {
  if (url && (url.startsWith('http://') || url.startsWith('https://'))) {
    try {
      ...
      console.error(
        `Assicurati che il backend sia in esecuzione (es. localhost:3001), oppure rimuovi/commenta OPENAPI_SPEC_URL dal file .env per usare il mock di default.\n`
      );
      process.exit(1);
    }
  } else if (url) {
